...
namespace Gamekit2D
{
    ...
    public class MovingPlatform : MonoBehaviour
    {
        ...
        private Transform down;//停止地点—下
        private Transform up;//停止地点—上
        private Vector2 down_stop;//停止坐标—下
        private Vector2 up_stop;//停止坐标—上
        ...
        private void Awake()
        {
            ...
            {
                if (child.name == "down_stop")
                {
                    down = child;
                    down_stop = child.GetComponent<Transform>().position;

                }

                if (child.name == "up_stop")
                {
                    up = child;
                    up_stop = child.GetComponent<Transform>().position;
        ...
        private void FixedUpdate()
        {
            if (down)
            {
                if (this.transform.position.y <= down_stop.y)
                {
                    speeder = 0;
                }
            }
            if (up)
            {
                if (this.transform.position.y >= up_stop.y)
                {
                    speeder = 0;
                }
            }
            ...
            float distanceToGo = speeder * Time.deltaTime;
